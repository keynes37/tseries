...
::: incremental
- Con la [heterocedasticidad]{.bg style="--col: #00FFFF"} los estimadores siguen siendo **lineales** e insesgados pero ya no con mínima varianza.
$$Var(\epsilon_{t}|X)=\sigma^{2}_{i}; \ con \quad i=1,2,...,n$$
- Suponga que tiene un modelo `r fa("long-arrow-alt-right", fill="red")`AR, de tal forma:
    $$Y_{t}= \phi Y_{t-1}+\epsilon_{t}, \quad |\phi| < 1$$
- Se encuentra que su valor esperado (esperanza) y su varianza será:
$$E(Y)=0 \quad Var= \frac{\sigma^{2}_{\epsilon}}{(1-\phi^{2})}$$
:::
...
$$Y_{t}= \phi Y_{t-1}+\epsilon_{t}, \quad |\phi| < 1$$
- Si su `r fa("mug-hot", fill="red")` varianza es:
$$Var_{t-1}\epsilon_{t}=h^{2}_{t}= \alpha_{0}+\alpha_{1}\epsilon^{2}_{t-1}$$
- Lo *anterior* puede ser reescrito como:
$$h^{2}_{t}-\sigma^{2}= \alpha_{1}(\epsilon^{2}_{t-1}-\sigma^{2})$$
:::
:::
...
- Una vez estimado el modelo en media (solo con constante), procedemos aplicar el [Test de Lagrange]{.fg style="--col: #FF0000"}
$$(T-q)R^2 \sim \chi^2_{(1-\alpha, q)}$$
- El cual contrasta a:
$$H_0: \gamma_1\neq\gamma_2\neq0$$
- Al estimar vamos a tener la siguiente [ecuación]{.bg style="--col: #FFFF00"}
$$\varepsilon_t^2=\gamma_0+\gamma_1\varepsilon_{t-1}^2$$
...
Lo cual nos arroja un `r LM` < `r Chicr``, que es prácticamente cero. *Note la notación científica*. Para decir que nuestra serie posee efectos [ARCH]{.fg style="--col: #FF0000"} este valor debe ser siempre **mayor**.
