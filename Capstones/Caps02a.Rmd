...
date: "<br> **Fecha Límite**: Sábado, 27 de septiembre de 2025"
output: 
...
.mono[**ENTREGABLE**]: El proyecto debe ser realizado máximo por cuatro (4) personas. Para las **siguientes** preguntas conteste solo lo que se le pide. **No se acepta** ningún trabajo adicional a la fecha del _sábado 27 _ de septiembre hasta la hora de las 23:59 en la carpeta del brightspace disponible para esto y cuya entrega solo debe hacerla un/a (1) de los integrantes del grupo.
...
- $y_t=a+0.3y_{t-1}+0.1y_{t-2}+e_t$
- $y_t=a+0.22y_{t-1}+0.64y_{t-2}+e_t$

**2b.** Mire estos ARMA y determine si son o no estacionarios y si pueden ser invertidos:

- $y_t=0.28y_{t-1}+0.15y_{t-2}+ 0.10e_{t-1}+0.6e_{t-2}+e_t$
- $y_t=1.42-0.28y_{t-1}+0.44y_{t-2}+0.35e_{t-1}-0.45e_{t-2}+e_t$
...
**3a.** Genere un proceso $y_t=0.9+0.25y_{t-1}+0.15y_{t-2}+0.38e_{t-1}+e_t$ cuando $e_t\sim (0, 2.5)$ para 400 observaciones.
...
## Problema 5: Portafolio

Tome 3 acciones que le gusten. Puede usar las de sus ejercicios anteriores de Arch y Garch

**5a.** Calcule los retornos mensuales y el riesgo (desviación estándar) de cada acción.

**5b.** Construya un portafolio mínimo riesgo con esas acciones, imponiendo que el peso total sume 100% y que no haya pesos negativos.
